...
document.querySelector("#theme-toggle").addEventListener("click", (e) => {
  ...
  if (e.button === 0) userSettings.theme.next();
  else userSettings.theme.prev();
});
...
document.getElementsByName(
  `region-${userSettings.regionPreference.currentValue}`,
)[0].selected = true;
...
document
  .querySelector("#decomposed-characters")
  .addEventListener("click", decomposedCharacterClicked);
...
function initPrac() {
  ...
  document
    .querySelectorAll(keyboardKeyClasses.blink)
    .forEach((key) => key.classList.remove(keyboardKeyClasses.blink));

  const char = getCangjieCharacter();
  const charCode = getCangjieCode();

  if (typeof charCode === "object") {
    // char has regional differences
  ...
  decomposedChars
    .slice(appState.testCharCodeLength)
    .forEach((unusedCursorChar) => (unusedCursorChar.style.display = "none"));
...
function handleKeyInput(e) {
  ...
  if (!appState.incrementCodePosition())
    // this check also increments the code position
    initPrac();
...
function layoutHandleInput(keyname = "") {
  ...
  appState.currentDecomposedChar.classList.remove(
    decomposedCharClasses.selected,
  );
...
function decompositionHandleInput(keyname = "") {
  ...
  if (keyname === " ") {
    if (
      ...
      appState.currentDecomposedChar.textContent =
        keyToRadicalTable[appState.currentChar];
      appState.currentDecomposedChar.classList.add(decomposedCharClasses.faded);
    }
...
function decomposedCharacterClicked(e) {
  ...
  if (
    ...
    decomposedChar.textContent =
      keyToRadicalTable[appState.testCharCode[characterIndex]];
    decomposedChar.classList.add(decomposedCharClasses.faded);
  }
}

function handleKeyRelease(e) {
  if (userSettings.modeValue !== "layout") return;

  const keyname =
    e.type === "keyup" ? e.key.toLowerCase() : e.target.id.slice(-1);
