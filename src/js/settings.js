...
class Setting extends Cycler {
  ...
  constructor(values = [], saveParams) {
    ...
    if (
      saveParams.length !== 2
      || typeof saveParams[0] !== "string"
      || typeof saveParams[1] !== "boolean"
    )
...
class Settings {
  ...
  constructor() {
    ...
    this.#theme = new Setting(
      ["light", "dark", "forest", "ocean", "apple", "ice", "fire", "royalty"],
      ["theme", true],
    );
    this.#regionPreference = new Setting(
      ["hk", "tw"],
      ["regionPreference", false],
    );
    this.#mode = new Setting(["layout", "decomposition"], ["mode", true]);
    this.#kbVisibility = new Setting(
      ["visible", "hidden"],
      ["kbVisibility", true],
    );
  }
...
userSettings.regionPreference.setByValue(
  loadSetting("regionPreference", userSettings.regionPreferenceValue)
);
userSettings.kbVisibility.setByValue(
  loadSetting("kbVisibility", userSettings.kbVisibilityValue)
);
